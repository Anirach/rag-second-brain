...
Vocabulary tracker file: `memory/vocabulary.md`. All word data lives there.

## Reading Workflow

When the user wants to read/practice reading a book:
1. Ask what book they're reading
2. Check `memory/vocabulary.md` for words from that book — **always check the END of the Active Words section** (use `tail` or read the last entries) to find the actual last word. Do NOT rely on `memory_search` for this — it returns partial/ranked results and may miss the latest entry.
...
4. **Add to tracker** immediately (never ask "should I add it?")
...
- User provides audio clips → save to `docs/tts-fr/`, add **Audio** field with path
