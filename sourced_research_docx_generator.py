#!/usr/bin/env python3
"""
Professional DOCX Generator with SOURCE CITATIONS and HYPERLINKS
- Proper Table of Contents with bookmarks
...
class SourcedResearchDocumentGenerator:
    def __init__(self):
        self.doc = Document()
        self._setup_styles()
    ...
    def add_hyperlink(self, paragraph, text, url):
        ...
        # This is a simplified hyperlink - in a full implementation,
        # you'd need more complex XML manipulation for proper hyperlinks
        run = paragraph.add_run(text)
        run.font.color.rgb = RGBColor(0, 0, 255)  # Blue color
        run.font.underline = True
        
        # Add URL in parentheses for now (since proper hyperlinks require complex XML)
    ...
    def add_professional_table(self, headers, rows, title=""):
        """Add table with source links in verification column"""
        if title:
        ...
        # Data rows with hyperlinks in last column
        for row_data in rows:
            ...
            for i, cell_data in enumerate(row_data):
                if i == len(row_data) - 1 and 'http' in str(cell_data):  # Last column with URL
                    ...
                    cell_para.clear()
                    self.add_hyperlink(cell_para, "View Source", str(cell_data))
                else:
    ...
    def add_sources_section(self, sources_data):
        ...
        if 'primary_sources' in sources_data:
            ...
            for i, source in enumerate(sources_data['primary_sources'], 1):
                ...
                self.add_hyperlink(url_para, source['url'], source['url'])
...
def create_sourced_research_report(title, data):
    """Create research report with proper source citations"""
    generator = SourcedResearchDocumentGenerator()
...
def main():
    ...
    if len(sys.argv) < 4:
        print("Usage: python3 sourced_research_docx_generator.py <title> <config.json> <output.docx>")
        sys.exit(1)
    ...
    generator = create_sourced_research_report(title, data)
    filename = generator.save(output_file)
    print(f"✅ Research report with SOURCE CITATIONS created: {filename}")
