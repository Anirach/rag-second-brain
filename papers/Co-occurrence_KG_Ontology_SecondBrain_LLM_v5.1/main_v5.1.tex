...
\textbf{FE2H} \cite{tu2022fe2h} represents extractive multi-hop pipelines that perform explicit graph reasoning over evidence spans. Unlike RAG-centric approaches that retrieve and generate, FE2H extracts answer spans through hierarchical evidence selection. Our approach differs fundamentally: we augment generation with retrieved evidence rather than performing span extraction, enabling more flexible answer synthesis while maintaining structured reasoning through ontology constraints.}
...
\begin{table}[t]
\centering
\caption{Main results with 95\% bootstrap confidence intervals. Faith.~= Faithfulness score (0-100).}
\label{tab:main}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Method} & \textbf{HotpotQA} & \textbf{CWQ} & \textbf{WebQ} & \textbf{Faith.} \\
& EM & F1 & EM & \\
\midrule
BM25 & 41.2$_{\pm0.8}$ & 38.4$_{\pm0.9}$ & 35.6$_{\pm1.1}$ & 68.2 \\
DPR & 48.6$_{\pm0.7}$ & 44.2$_{\pm0.8}$ & 42.1$_{\pm0.9}$ & 71.4 \\
ColBERT & 52.1$_{\pm0.6}$ & 47.8$_{\pm0.7}$ & 45.3$_{\pm0.8}$ & 73.8 \\
GraphRAG & 58.3$_{\pm0.5}$ & 51.2$_{\pm0.6}$ & 49.8$_{\pm0.7}$ & 78.6 \\
\midrule
\textbf{Ours} & \textbf{62.7}$_{\pm0.4}$ & \textbf{55.8}$_{\pm0.5}$ & \textbf{53.2}$_{\pm0.6}$ & \textbf{84.2} \\
\bottomrule
\end{tabular}
\end{table}

Faithfulness improvements (+5.6 over GraphRAG) stem from ontology-grounded constraints that filter inconsistent evidence before generation.
...
\begin{table}[t]
\centering
\caption{Comparison with recent systems (HotpotQA). Faith.~= Faithfulness.}
\label{tab:recent_results}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Method} & \textbf{EM} & \textbf{Faith.} & \textbf{Latency (s)} \\
\midrule
AMKOR \cite{fang2024amkor} & 60.2 & 79.1 & 2.8 \\
KGA \cite{wang2024kga} & 57.8 & 76.4 & 1.4 \\
FAIR-RAG \cite{chen2024fairrag} & 59.1 & 81.3 & 2.1 \\
MA-RAG \cite{li2024marag} & 61.4 & 80.2 & 3.5 \\
\revisioncritical{PRISM \cite{chen2024prism}} & 60.8 & 78.9 & 2.6 \\
\midrule
\textbf{Ours} & \textbf{62.7} & \textbf{84.2} & \textbf{1.8} \\
\bottomrule
\end{tabular}
\end{table}
...
Our knowledge graphs are constructed from publicly available sources:
\begin{itemize}
    \item \textbf{Wikidata}: CC0 license (public domain), 1.2M entities, 4.8M triples
    \item \textbf{ConceptNet}: CC-BY-SA 4.0 license, 850K entities for commonsense reasoning
\end{itemize}
Both sources permit academic and commercial use with appropriate attribution.

\subsection{Potential Biases}
Knowledge graphs inherit biases from their sources:
\begin{itemize}
    \item \textbf{Coverage bias}: English Wikipedia over-represents Western topics; entity linking may fail on underrepresented domains
    \item \textbf{Temporal bias}: KG snapshots reflect knowledge at creation time; recent events may be missing
    \item \textbf{Ontology design}: Our OWL 2 RL schema reflects authors' domain modeling choices
\end{itemize}

\subsection{Maintenance and Deployment Considerations}
Practical deployment requires:
\begin{itemize}
    \item Periodic KG updates (we recommend quarterly refreshes)
    \item Monitoring for entity linking failures on new domains
    \item Ontology extension for domain-specific applications
\end{itemize}

The approach is particularly suited for high-stakes, correctness-critical domains (e.g., biomedicine, legal) where deductive reasoning over curated knowledge provides auditable evidence trails.
