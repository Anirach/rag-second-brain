% NCD-CIE v21 — Integrated experimental results (Feb 2026)
...
Cross-sectional validation on T2DM (AUC\,=\,0.68) and CKD
(AUC\,=\,0.67) confirms multi-NCD generalisability.
...
Inoue et al.~\cite{inoue2023causalforest} demonstrate causal forest
for estimating individualised treatment effects (ITE) in blood pressure
management using RCT data (SPRINT, ACCORD-BP); while their approach
achieves personalised benefit prediction, it remains a data-driven
black-box ensemble without interpretable causal pathways---NCD-CIE
complements such methods by providing transparent causal reasoning
via expert-curated knowledge graphs.
...
search over $\gamma \in [0.3, 1.0]$ minimising RMSE against six
RCT benchmarks yields $\gamma^* = 0.65$ (RMSE\,=\,0.74);
...
White American cohort; the NL parser and explanation generator lack quantitative
evaluation (a 50-query test suite has been developed; a clinician
user study is planned);
...
and cross-sectional T2DM/CKD validation (AUC\,0.67--0.68)
provide evidence that expert-curated causal structure enables
clinically plausible what-if reasoning across multiple NCD
endpoints.
