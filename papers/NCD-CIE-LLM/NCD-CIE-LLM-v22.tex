% NCD-CIE v22 — Major revision: Causal-Informed Decision Support + HTE (Feb 2026)
...
performance.
Cross-sectional validation confirms multi-NCD generalisability
(T2DM AUC\,=\,0.68; CKD AUC\,=\,0.67).
...
(Section~\ref{sec:hte}).
\textbf{(C3)}~Three LLM augmentation layers---graph validation
...
\textbf{(C4)}~Multi-source evaluation including cross-population
...
and transportability validation on non-Western cohorts.
...
clinically plausible what-if reasoning and HTE estimation across
multiple NCD endpoints.
