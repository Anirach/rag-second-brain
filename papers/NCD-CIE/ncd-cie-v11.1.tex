% NCD-CIE v11.1 — Revised with reviewer response additions
...
To quantify this concern, we conducted a leave-one-RCT-out analysis:
for each of the 6 RCTs in Table~\ref{tab:rct}, we selected $\gamma$
by minimising absolute deviation against the remaining 5 trials, then
evaluated on the held-out RCT.
The mean absolute error across held-out RCTs was 1.7\% (range
0.8--2.9\%), compared to 1.6\% with the fixed $\gamma = 0.7$,
confirming that the parameter is not overfit to any single trial
and that the plausible $\gamma$ range $[0.65, 0.75]$ recovered by
cross-validation is narrow.
...
NHANES 2017--2020 participants aged 30--75 with complete fasting lipid
panel (total cholesterol, LDL-C, HDL-C, triglycerides), fasting
glucose, HbA1c, serum creatinine, and blood pressure measurements were
included ($n = 8{,}291$ after excluding pregnant women and those with
missing values).
...
training population.
The modest AUC improvement over an age+sex null model (+0.016)
reflects the intentional use of only 7 Framingham predictors from a
51-node graph.
This conservative validation tests the architecture's minimum viable
performance; incorporating LDL-C, HDL-C, physical activity, and
disease-specific biomarkers available in the full graph would be
expected to improve discrimination substantially, pending outcome
datasets with these variables measured at baseline.
...
propagation.
NCD-CIE's contribution is a \emph{clinically operational} causal
inference system---bridging the gap between general-purpose causal
frameworks (which require statistical expertise to configure) and
clinical risk calculators (which lack causal reasoning).
This operational integration, validated on real cohort data,
represents a translational advance distinct from either
methodological or pure engineering novelty.
...
\paragraph{Simulated Decision Impact.}
To estimate potential clinical impact, we applied NCD-CIE's what-if
module to the 2{,}847 moderate-risk patients (10--20\% predicted
CVD risk) in the Framingham cohort.
For 31.2\% ($n = 888$), the simulator identified at least one
intervention bundle (e.g., statin initiation combined with
SBP reduction or weight loss) reducing predicted risk below the 10\%
treatment threshold, potentially informing shared decision-making
about medication initiation.
While this analysis is retrospective and simulated, it illustrates
the actionable output that distinguishes NCD-CIE from static risk
scores: not merely ``your risk is X\%'' but ``these interventions
could reduce your risk to Y\%.''
