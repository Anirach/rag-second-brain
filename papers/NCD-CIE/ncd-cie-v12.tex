...
Direct KG-weight validation on Framingham yields AUC\,=\,0.672
(blended 0.688), and interventional validation on the Diabetes\,130
US Hospitals dataset ($n\!=\!101{,}766$ encounters) shows 100\%
directional accuracy across 10 medications ($r\!=\!0.54$).
...
To contextualise, the largest known unmeasured CVD confounder---lipoprotein(a)---has
RR\,$\approx$\,1.5~\cite{erqou2009lpa}, well below the E-value
threshold of 2.4.
Even combining Lp(a) with socioeconomic deprivation
(RR\,$\approx$\,1.3~\cite{stringhini2017socioeconomic}), the joint
confounding strength ($\sim$1.95) remains below the bound,
suggesting the causal estimates are robust to the most plausible
omitted variables.
...
constrained range.
Table~\ref{tab:sensitivity} shows the statin effect spans
$-$3.9\% to $-$8.1\% across $\gamma \in [0.5, 0.9]$.
Critically, the clinical recommendation (initiate statin therapy)
...
propagation.
NCD-CIE bridges this gap as a \emph{clinically operational}
causal inference system---translating general-purpose causal
methods into a tool that clinicians can use without statistical
expertise, while preserving formal causal semantics.

\subsection{Direct KG Validation on Framingham}\label{sec:directkg}

To test the causal graph's \emph{intrinsic} discriminative power
(independent of SCORE2 or D'Agostino coefficients), we computed
composite risk scores directly from KG edge weights applied to
$n = 4{,}240$ Framingham participants.
Each patient's risk factors were mapped to binary KG nodes
(e.g., SBP\,$\geq$\,140 or prevalent hypertension $\to$
\texttt{hypertension}; totChol\,$\geq$\,240 $\to$
\texttt{high\_cholesterol}), and the composite score
$\hat{R} = \sum_j w_j \cdot x_j$ was evaluated against the
10-year CHD outcome.

The KG-only score achieved AUC\,=\,0.672, confirming that the
expert-curated edge weights alone capture meaningful
cardiovascular risk signal well above chance.
Blending with a logistic regression baseline
($\gamma\,=\,0.7$: KG 70\%, statistical 30\%) yielded
AUC\,=\,0.688.
The gap relative to the coefficient-fitted models
(SCORE2: 0.704; D'Agostino: 0.721) is expected: the KG uses
coarse binary mappings and only 8 weighted edges, whereas the
reference models use optimised continuous coefficients.
Gamma sensitivity analysis showed AUC ranging from 0.672
($\gamma\!=\!1.0$, KG-only) to 0.714 ($\gamma\!=\!0.3$),
confirming the KG provides complementary signal to statistical
models.

\subsection{Interventional Validation on Diabetes\,130 Dataset}\label{sec:diab130}

To validate the what-if simulation component, we used the
Diabetes\,130 US Hospitals dataset~\cite{strack2014diabetes130}
($n = 101{,}766$ encounters; $71{,}518$ patients), identifying
$16{,}773$ patients with $\geq$\,2 visits and $25{,}968$
medication changes between consecutive encounters.

For each of 10 diabetes medications, we compared the
KG-predicted effect of initiating therapy against the
\emph{observed} change in readmission rate between visits
(Table~\ref{tab:diab130}).
All 10 medications showed correct directional alignment
(100\% direction accuracy, $p < 0.001$ for 9 of 10 by
paired $t$-test).
The overall Pearson correlation between predicted and observed
effect sizes was $r = 0.54$ ($p = 0.10$).

The KG consistently underestimates effect magnitude (predicted
$-$2.0 to $-$8.0\% vs.\ observed $-$19 to $-$43\%), which
reflects the conservative design of the causal weights and
confounding in the observational data (patients who start
medications may have been sicker at baseline).
The key finding is \emph{directional fidelity}: the KG
correctly identifies which interventions reduce risk, which
is the clinically actionable output of what-if queries.

\renewcommand{\arraystretch}{1.1}
\begin{table}[t]
\centering
\caption{What-if validation: KG-predicted vs.\ observed readmission
rate change upon medication initiation
(Diabetes\,130 dataset).}\label{tab:diab130}
\small
\begin{tabular}{@{}lrccc@{}}
\toprule
\textbf{Medication} & $n$ & \textbf{Predicted} & \textbf{Observed}
& \textbf{Dir.} \\
\midrule
Metformin    & 1{,}819 & $-$8.0\% & $-$42.6\% & \checkmark \\
Insulin      & 3{,}922 & $-$5.0\% & $-$40.5\% & \checkmark \\
Glipizide    & 1{,}121 & $-$4.0\% & $-$37.5\% & \checkmark \\
Glyburide    &     863 & $-$3.0\% & $-$43.0\% & \checkmark \\
Pioglitazone &     727 & $-$3.5\% & $-$41.0\% & \checkmark \\
Rosiglitazone &    565 & $-$3.0\% & $-$36.6\% & \checkmark \\
Glimepiride  &     518 & $-$3.5\% & $-$36.5\% & \checkmark \\
Repaglinide  &     286 & $-$2.5\% & $-$39.9\% & \checkmark \\
Nateglinide  &      92 & $-$2.0\% & $-$29.3\% & \checkmark \\
Acarbose     &      42 & $-$2.0\% & $-$19.0\% & \checkmark \\
\midrule
\multicolumn{2}{@{}l}{Correlation} & \multicolumn{3}{c}{$r = 0.54$, direction accuracy 10/10} \\
\bottomrule
\end{tabular}
\end{table}
\renewcommand{\arraystretch}{1.0}
...
the graph captures meaningful risk signal without coefficient fitting,
and interventional validation on the Diabetes\,130 dataset shows
100\% directional accuracy across 10 medications ($r = 0.54$),
demonstrating that the what-if simulator correctly identifies
...
calibration slope\,=\,0.91), direct KG-weight validation
(AUC\,=\,0.672), interventional validation on Diabetes\,130
(100\% directional accuracy, 10 medications), six RCT comparisons,
and structural validation via the PC algorithm (64.5\% shared edges).
