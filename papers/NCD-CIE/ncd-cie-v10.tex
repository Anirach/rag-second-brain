...
All coefficients came from published literature with zero data
fitting~\cite{dagostino2008}.
This is comparable to the original Framingham Risk Score's C-statistic
of ${\sim}$0.75~\cite{dagostino2008}, which was \emph{fit} to
this same cohort.
...
The Framingham validation primarily confirms correct implementation
of established risk coefficients.
Because the same published coefficients serve as edge weights, the
AUC of 0.721 demonstrates faithful reproduction of their predictive
power.
It does not, by itself, validate the causal knowledge graph structure
or the what-if simulator.
