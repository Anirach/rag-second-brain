...
Direct KG-weight validation yields AUC\,=\,0.672, and
observational consistency checks on the Diabetes\,130 dataset
($n\!=\!101{,}766$) show directional alignment for 10 medications.
...
unmeasured CVD confounder (Lp(a), RR\,$\approx$\,1.5~\cite{erqou2009lpa}).
...
\end{scope}
\end{tikzpicture}
\caption{Framingham validation ($n\!=\!4{,}172$):
(a)~calibration by decile (slope\,=\,0.91, intercept\,=\,$-$0.08);
(b)~ROC curve (AUC\,=\,0.721 [0.700--0.741]).}\label{fig:calibration_roc}
\end{figure}
...
\paragraph{Observational Consistency on Diabetes\,130.}\label{sec:diab130}
As an ecological consistency check, we applied the what-if
component to the Diabetes\,130 US Hospitals
dataset~\cite{strack2014diabetes130}
($n\!=\!101{,}766$ encounters; $16{,}773$ multi-visit patients;
$25{,}968$ medication changes).
For 10 diabetes medications, KG-predicted effects of therapy
initiation were compared against observed readmission changes.
All 10 showed correct directional alignment; Pearson
$r = 0.54$ ($p = 0.10$, not significant; Spearman $\rho = 0.52$).
The KG substantially underestimates magnitude (predicted $-$2 to
$-$8\% vs.\ observed $-$19 to $-$43\%), reflecting residual
confounding by indication, concurrent interventions, and regression
to the mean---no RCT reports readmission reductions of this
magnitude from single-drug initiation.
This is an \emph{associational} consistency check, not causal
validation; the RCT comparisons (Table~\ref{tab:rct}) provide
stronger interventional evidence.
...
edge weights can be recalibrated for Thai/Asian populations using
EGAT~\cite{sritara2003egat} and Thai NHES~\cite{aekplakorn2011thai}
data while preserving graph structure.
...
and observational consistency checks on Diabetes\,130 show
correct directional alignment for all 10 tested medications
($r = 0.54$, $p = 0.10$), though this associational evidence
is weaker than the RCT comparisons.
...
observational consistency on Diabetes\,130 (10 medications),
