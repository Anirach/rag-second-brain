% NCD-CIE v10.4 — Preventive healthcare reframing of v10.3
...
\titlerunning{NCD-CIE: Predictive and Preventive NCD Healthcare}

\begin{document}

\title{NCD-CIE: A Causal Knowledge Graph Engine\\for Predictive and Preventive\\Non-Communicable Disease Healthcare}
...
mortality, yet preventive healthcare remains reactive---clinicians
identify risk but lack tools to simulate how specific interventions
would alter patient trajectories.
We present \textbf{NCD-CIE}, an open-source predictive and preventive
healthcare engine with three components:
...
\keywords{Preventive healthcare \and Causal inference \and Knowledge graph \and
Non-communicable disease \and Risk prediction \and
Interventional simulation \and Clinical decision support}
\end{abstract}
...
current tools share a fundamental limitation that impedes preventive care:
they estimate \emph{associational} risk but cannot reason about
\emph{causal} mechanisms or simulate the effect of preventive interventions
before they are applied.

Consider a preventive care scenario: a clinician asks,
``What would this patient's 10-year CVD risk become if they lowered
LDL-C by 1\,mmol/L and lost 5\,kg?''
This predict-then-prevent reasoning is central to personalised
preventive healthcare, yet neither Framingham nor QRISK3 can answer it.
Doing so requires moving beyond rung~1 (association) of Pearl's
causation ladder~\cite{pearl2018book} to rung~2 (intervention),
where do-calculus enables reasoning about the effects of preventive
...
\item A \textbf{what-if simulator} for preventive decision support
  that propagates interventional changes through the causal knowledge
  graph using topological sorting, enabling clinicians to evaluate
  the predicted impact of preventive actions before implementation.
\end{enumerate}

\noindent\textbf{Translational contribution.}
Existing tools either predict risk (Framingham, SCORE2) or model
causation (DoWhy, CausalNex), but none close the
\emph{predict--simulate--prevent} loop needed for preventive healthcare.
NCD-CIE is, to our knowledge, the first system to \emph{jointly}
combine four capabilities into a single preventive decision-support
tool.
...
prediction horizon.
To assess the sensitivity of this assumption, we compared the
independence-based composite against a Gaussian copula model
incorporating pairwise correlations
$\rho_{\text{CVD,T2DM}} = \rho_{\text{CVD,CKD}} = \rho_{\text{T2DM,CKD}} = 0.3$,
reflecting shared upstream risk factors (obesity, hypertension,
inflammation).
On the NHANES cohort ($n = 8{,}291$), the Spearman rank correlation
between the independence-based and copula-based composite scores was
$r_s = 0.97$, with fewer than 3.8\% of individuals changing risk
quintile.
Even at $\rho = 0.5$ (an upper bound given the partially
non-overlapping predictor sets across endpoints), rank agreement
remained high ($r_s = 0.94$, 5.1\% quintile changes).
The independence assumption thus introduces minimal ranking distortion
for clinical triage purposes, though it may modestly underestimate
absolute joint risk in patients with strongly correlated risk
profiles.
...
We set $\gamma = 0.7$ based on a literature review of biological
signal decay rates.
...
$\gamma \in [0.6, 0.8]$, with 0.7 at the midpoint.
...
Future work should pre-register the $\gamma$ selection protocol or
learn $\gamma$ from held-out data using cross-validation to avoid
any risk of post-hoc tuning.
The default $d_{\max} = 3$ was similarly chosen to limit
...
A synthetic cohort ($n = 5{,}000$, 70/30 split) generated from the
graph's own structural equations confirms implementation correctness
(Table~\ref{tab:synthetic}): C-statistics 0.76--0.82, calibration
slopes 0.89--0.93.
...
As the primary outcome-based test, we replaced the CVD risk
...
evaluating on the Framingham cohort via 5-fold cross-validation yielded
AUC-ROC\,=\,0.704 [0.682--0.726], calibration slope\,=\,0.87
[0.79--0.95], and E:O\,=\,1.08 [1.00--1.16].
This result is free of any coefficient--cohort circularity:
the SCORE2 coefficients were derived from European populations,
and no parameter was tuned to Framingham outcomes.
The clinically acceptable discrimination (AUC\,=\,0.704) despite
population mismatch (European vs.\ US cohort) demonstrates
that the KG architecture's risk stratification ability generalises
beyond its training population.

\paragraph{Confirmatory Analysis with D'Agostino Coefficients.}
As a confirmatory check, we also evaluated using D'Agostino
coefficients~\cite{dagostino2008}, which were originally derived from
a Framingham cohort.
NCD-CIE achieved AUC-ROC\,=\,0.721 [95\% CI: 0.700--0.741]
(Fig.~\ref{fig:roc}) and
Brier score\,=\,0.118 [0.113--0.124].
The modest AUC improvement over SCORE2 (0.721 vs.\ 0.704, DeLong
$p = 0.18$) is not statistically significant and likely reflects
partial coefficient--cohort matching rather than superior architecture
performance.
We therefore treat the SCORE2 result (AUC\,=\,0.704) as the more
conservative and methodologically appropriate primary estimate.
Table~\ref{tab:framingham_sub} reports subgroup performance using
D'Agostino coefficients.
...
\paragraph{E-Value Sensitivity Analysis for Unmeasured Confounding.}
The causal sufficiency assumption---that all common causes are
included in the graph---is unlikely to hold perfectly in practice.
Following VanderWeele and Ding~\cite{vanderweele2017evalue}, we
computed E-values to quantify the minimum strength of association
that an unmeasured confounder would need with both the exposure and
outcome to explain away key findings.
For the overall discrimination result (AUC\,=\,0.721, corresponding
to an approximate risk ratio of RR\,$\approx$\,1.8 for the top vs.\
bottom risk quintile), the E-value is 3.0, meaning an unmeasured
confounder would need to be associated with both risk classification
and CHD outcome by a factor of $\geq$3.0 each to reduce the observed
association to null.
For the statin intervention effect (simulated ARR\,=\,5.8\%,
RR\,$\approx$\,1.5), the E-value is 2.4 (lower 95\% CI bound:
1.8).
Given that the strongest known unmeasured confounders in
cardiovascular epidemiology (e.g., lipoprotein(a), social deprivation
indices) typically exhibit risk ratios of 1.3--1.8 with CHD outcomes,
these E-values suggest that our findings are moderately robust to
unmeasured confounding, though not immune to a single strong
confounder acting on both exposure and outcome simultaneously.
...
The GAM comparison validates linearity for 7 predictors; extending
this assessment to the full 50-predictor system requires outcome data
beyond the Framingham subset and is a priority for future validation.
...
(2)~clinical guardrails for physiologically implausible values;
...
(NHANES via CDC, Framingham via BioLINCC); no IRB approval was required.

%==============================================================
\section{Discussion}\label{sec:discussion}

NCD-CIE demonstrates that a causal knowledge graph with logistic-link
scoring and topological intervention propagation can serve as a
foundation for predictive and preventive healthcare.
The system closes the gap between static risk estimation and
actionable prevention: it predicts individual risk, simulates
the effect of specific interventions, and quantifies the expected
benefit---enabling a \emph{predict--simulate--prevent} clinical workflow.
Predictions closely agree with established models ($r = 0.91$),
intervention simulations align with RCT outcomes, and the causal
architecture provides mechanistic transparency.

\paragraph{Validation Scope and Strengths.}
The primary outcome-based validation uses SCORE2 coefficients
(AUC\,=\,0.704), which are fully independent of the Framingham
cohort, confirming that the KG architecture generalises across
populations.
The confirmatory D'Agostino analysis (AUC\,=\,0.721) is consistent
but not independent, as these coefficients originate from
Framingham data.
Both configurations use only 7 of ${\sim}$50 available predictors
with zero data fitting.
...
E-value analysis (Section~\ref{sec:framingham}) indicates that
unmeasured confounders would need associations of RR\,$\geq$\,2.4--3.0
to nullify key findings, providing moderate robustness to violations
of the causal sufficiency assumption.

\paragraph{Limitations and Future Work.}
Key limitations include: rung-2 (interventional) only, not rung-3
counterfactual; linearity assumption validated for 7 predictors
but untested at full scale; expert curation is slow and static;
Framingham validation uses only 7 predictors on a predominantly
White American cohort; composite score independence assumption
($<$4\% ranking change at $\rho=0.3$, but absolute risk may be
underestimated); declining AUC with age (0.591 for $>$60);
and causal sufficiency cannot be guaranteed despite E-value bounds.
Priorities include prospective validation in preventive care settings
(EGAT, Thai NHES), EHR integration via FHIR to embed the
predict--simulate--prevent workflow into clinical practice,
competing-risk modelling, pre-registered $\gamma$ selection,
and rung-3 counterfactual reasoning.

%==============================================================
\section{Conclusion}\label{sec:conclusion}

We presented NCD-CIE, a causal knowledge graph engine for predictive
and preventive NCD healthcare, operating at rung~2 of Pearl's
causation ladder.
By unifying risk prediction with interventional simulation, NCD-CIE
enables the \emph{predict--simulate--prevent} workflow that preventive
medicine requires but current tools cannot support.
...
specification in Appendix~\ref{app:edges}, providing a reproducible
foundation for predictive and preventive NCD healthcare research.
