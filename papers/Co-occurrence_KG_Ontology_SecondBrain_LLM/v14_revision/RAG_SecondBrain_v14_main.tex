...
% Version 14 - Major revision: restructured sections, added algorithm details
% February 2026
...
We provide empirical validation on the HotpotQA benchmark, demonstrating an 8.5\% improvement in Recall@10 over the best single-source retriever. This validates our multi-source hypothesis while also serving as an architectural blueprint for future extensions.
...
\subsection{Recent Advances in Multi-Hop and Agentic RAG (2024--2026)}

Several recent systems address challenges directly relevant to our framework:

\textbf{Beam Retrieval} \citep{beam2024} pioneers end-to-end multi-hop retrieval with learned beam search, achieving state-of-the-art on HotpotQA (78.2\% EM) and 2WikiMQA. \textit{Relation to our work:} Beam Retrieval learns retrieval chains jointly; our approach uses fixed multi-source fusion but could incorporate learned chain scoring.

\textbf{DualRAG} \citep{dualrag2025} implements dual-process reasoning with targeted retrieval, combining fast heuristic and slow deliberative retrieval modes. \textit{Relation to our work:} Our cost-aware gating relates conceptually but lacks the explicit dual-process architecture; integrating fast/slow pathways could improve efficiency.

\textbf{EVO-RAG} \citep{evorag2025} and \textbf{FrugalRAG} \citep{frugalrag2025} optimize retrieval policies through reinforcement learning, learning when to stop retrieving and how to balance cost-accuracy trade-offs. \textit{Relation to our work:} Our gating mechanism is trained supervised; RL-based optimization of gating weights is a promising extension.

\textbf{PRISM} \citep{prism2025} and \textbf{RT-RAG} \citep{rtrag2026} use agentic and tree-structured decomposition to balance precision and recall dynamically. \textit{Relation to our work:} Our cross-attention fusion could benefit from explicit query decomposition; these methods suggest planning as a pre-retrieval step.

\textbf{HyperbolicRAG} \citep{hyprag2025} demonstrates dual-space fusion combining Euclidean and hyperbolic embeddings to capture hierarchical structure. \textit{Relation to our work:} Our KG+OWL component addresses hierarchy symbolically; geometric + symbolic fusion is an interesting hybrid direction.

\textbf{FREESON} \citep{freeson2025} explores retriever-free traversal using LLM reasoning over fixed corpora. \textit{Relation to our work:} For small personal KBs, retriever-free approaches may be viable alternatives; we focus on scalable retrieval-based methods.

\textbf{HANRAG} \citep{hanrag2025} combines heuristic routing with noise filtering, achieving both accuracy and efficiency gains through learned routing policies. \textit{Relation to our work:} Our gating relates to their routing; explicit noise filtering in fusion is not currently addressed in our framework.

\textbf{TERAG} \citep{terag2025} demonstrates token-efficient graph construction and PPR-based retrieval, minimizing LLM token usage while maintaining strong recall. \textit{Relation to our work:} Our KG traversal uses similar PPR techniques; TERAG's token efficiency strategies are directly applicable to cost optimization.

\textbf{GFM-RAG} \citep{gfmrag2025} introduces a transferable GNN-based retriever that learns query-dependent graph traversal in a single forward pass. \textit{Relation to our work:} Our symbolic PPR+OWL approach trades learned flexibility for interpretability and explicit reasoning; GFM-RAG's learned traversal is a complementary paradigm.

\textbf{CatRAG} \citep{catrag2025} addresses the ``static graph fallacy'' with query-adaptive edge reweighting and symbolic anchoring for improved multi-hop coverage. \textit{Relation to our work:} Our PPR uses fixed edge weights with optional topic-sensitive biasing; query-conditional reweighting per CatRAG could improve our KG component.

\textbf{CompactRAG} \citep{compactrag2025} restructures corpora offline into atomic QA pairs, reducing online LLM calls while maintaining accuracy. \textit{Relation to our work:} For stable PKM systems, offline preprocessing is practical; CompactRAG's atomic decomposition could complement our entity-centric indexing.

\textbf{ToR (Tree of Retrieval)} \citep{tor2025} uses tree-structured iterative retrieval with explicit precision-recall balancing at each node. \textit{Relation to our work:} Our fusion is single-stage; ToR's iterative tree search with adaptive stopping provides a template for multi-round retrieval extensions.
...
These experiments use \textbf{proxy implementations}: BM25 for PPMI-based co-occurrence, and entity matching for full KG+OWL traversal. The full architecture with learned gating, cross-attention fusion, and ontological reasoning remains to be evaluated. Additionally, we report retrieval metrics only; end-to-end QA (EM/F1) evaluation is planned.
